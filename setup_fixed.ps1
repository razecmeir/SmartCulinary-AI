...
try {
    ...
    if ($pythonVersion -like "Python 3.9.*") {
        Write-Host "âœ“ Python 3.9.x already installed: $pythonVersion" -ForegroundColor Green
...
if (-not $pythonInstalled) {
    try {
        ...
        if ($pyVersion -like "Python 3.9.*") {
            Write-Host "âœ“ Python 3.9.x found via py launcher: $pyVersion" -ForegroundColor Green
            $pythonInstalled = $true
            $pythonPath = "py -3.9"
        }
    }
...
if (-not $pythonInstalled) {
    ...
    try {
        ...
        Write-Host "âœ— Failed to download Python installer" -ForegroundColor Red
    ...
    try {
        ...
        Write-Host "âœ— Python installation verification failed" -ForegroundColor Red
...
if ($env:VIRTUAL_ENV) {
    try {
        ...
        Write-Host "âœ“ Pip upgraded successfully inside venv" -ForegroundColor Green
    }
    catch {
    ...
    Write-Host "âœ— Virtual environment was not activated correctly" -ForegroundColor Red
    exit 1
}
...
if (-not (Test-Path $requirementsFile)) {
    Write-Host "âš  requirements.txt not found at: $requirementsFile" -ForegroundColor Yellow
...
try {
    ...
    Write-Host "âœ“ TensorFlow $tfVersion installed" -ForegroundColor Green
    ...
    if ($gpuCheck -like "*GPUs: 0*") {
        Write-Host "âš  No GPU detected - training will use CPU (slower)" -ForegroundColor Yellow
        Write-Host "  For GPU support, install CUDA Toolkit and cuDNN separately" -ForegroundColor Gray
    }
    else {
        Write-Host "âœ“ GPU detected ($gpuCheck) - Acceleration enabled!" -ForegroundColor Green
    }
}
catch {
    Write-Host "âš  TensorFlow import failed or not installed" -ForegroundColor Yellow
}
